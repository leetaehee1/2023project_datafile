...
def main():
    ...
    # 파일 불러오기
    path = "hogwarts_legacy_reviews_custom.csv"
    df = pd.read_csv(path, encoding_errors='ignore')#, encoding="cp949")
    data_X = df['Review'].values   # 문장 컬럼
    labels = df['Feedback'].values     # 라벨 컬럼
